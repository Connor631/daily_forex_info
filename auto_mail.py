...
from mail_process.loan_mail import get_loan
...
def job_loan_mail(sql_util, config):
    sender = config["mail_sender"]
    password = config["mail_sender_pwd"]
    receivers = config["mail_receiver"]
    revs = receivers.split(",")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--forex', type=int, default=0, help='外汇邮件任务是否启动')
    parser.add_argument('--stock', type=int, default=0, help='股市邮件任务是否启动')
    parser.add_argument('--mort_loan', type=int, default=1, help='股市邮件任务是否启动')
    ...
    if args.mort_loan:
        mort_tag = "mortgage"
        mort_sql = f"SELECT * FROM t_user_info WHERE uni_tag='{mort_tag}'"
        mortgage_mail_config = sql_util.read_sql(database="forex",sql=mort_sql, format="dict")
        # cron表达式
        mort_cron = mortgage_mail_config["sche_receiver"]
        scheduler.add_job(job_loan_mail, CronTrigger.from_crontab(mort_cron), kwargs={'sql_util': sql_util, 'config': mortgage_mail_config}, id='mortgage_job', misfire_grace_time=60)
        job_loan_mail(sql_util, mortgage_mail_config)
    else:
        logger.info("按揭贷款数据任务未启动")
